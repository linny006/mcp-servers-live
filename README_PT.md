...
> Índice de servidores MCP no GitHub, atualizado automaticamente a cada 15 minutos
...
Um tracker em tempo real que consulta a GitHub Search API a cada 15 minutos para descobrir, catalogar e ranquear novos servidores de Model Context Protocol (MCP). Diferente das awesome-lists mantidas manualmente, este projeto exibe atividade em tempo real —estrelas, commits e frescor dos releases— para você encontrar integrações MCP que estão de fato em desenvolvimento.
...
A cada 15 minutos, um GitHub Action executa o `tracker.py`. O script faz o seguinte:
